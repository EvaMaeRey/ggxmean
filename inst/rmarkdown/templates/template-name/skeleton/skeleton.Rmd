...
```{r ttest, include = T}
set.seed(1456)
ggplot(starwars %>% sample_n(50)) +
  aes(x = height) +
  geom_rug() +
  geom_dotplot() +
  ggxmean:::geom_ttestconf(size = 2, 
                           color = "mediumvioletred", 
                           alpha = .75, 
                           y = -.01, 
                           conf.level = .99) +
  ggxmean::geom_xmean(linetype = "dashed") +
  ggxmean:::geom_tdist(height = 1, alpha = .5, fill = "mediumvioletred")
```

---

`r chunk_reveal("ttest", break_type = 20)`
